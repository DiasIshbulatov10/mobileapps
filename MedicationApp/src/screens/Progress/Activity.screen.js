            ...
            <ScrollView>
                <View style={{backgroundColor: 'white'}}>
                    <List.Section style={{backgroundColor: 'white'}}>
                        <List.Subheader style={{color: basics.allStyle.common_btn_color}}>Popular activities</List.Subheader>
                        {
                            popularList.map((item, i) => {
                                return(
                                    <List.Item key={i} title={item.name} style={{paddingTop: 10, paddingBottom: 10}} onPress={() => navigation.navigate('ActivityDetail', {title: item.name})}/>
                                )
                            })
                        }
                    </List.Section>
                    <Divider bold={true}/>
                    <List.Section style={{backgroundColor: 'white'}}>
                        <List.Subheader style={{color: basics.allStyle.common_btn_color}}>All activities</List.Subheader>
                        {
                            !allShow ? 
